...
export default function CheckoutPage() {
  ...
  const validate = () => {
    if (!form.name || !form.phone || !form.email || !form.address) { alert("請填寫姓名、手機、Email 與收件地址。"); return false; }
  ...
  const placeOrder = async () => {
    ...
    setSubmitting(false);
    if (error) { console.error(error); alert(`訂單建立失敗：${error.message}`); return; }
  ...
  return <main className="min-h-screen bg-[#f4efe6] text-[#171512]">
    ...
    <section className="mx-auto max-w-7xl px-5 py-12 md:px-10 md:py-20"><p className="text-sm font-black tracking-[.28em] text-[#f05a19]">CHECKOUT PREVIEW</p><h1 className="mt-3 text-5xl font-black md:text-7xl">填寫訂購資料</h1><p className="mt-4 font-semibold text-black/55">資料會整理後帶到 LINE，由店家確認庫存、運費與付款方式。</p>
      <div className="mt-12 grid gap-10 lg:grid-cols-[1fr_420px]"><form className="grid gap-5 rounded-[2rem] border-2 border-black bg-[#fffaf1] p-6 shadow-[8px_8px_0_#171512] md:grid-cols-2 md:p-8">
        ...
        <label className="grid gap-2 text-sm font-black md:col-span-2">收件地址／超商門市<input value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} className="rounded-xl border-2 border-black bg-white px-4 py-3 font-medium" placeholder="地址或完整門市名稱" /></label>
